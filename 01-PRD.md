...
3. **As a support engineer**, I want to see similar past incidents and their resolutions, so I can reuse a known fix instead of investigating from zero.
4. **As a support engineer**, I want to see relevant KB articles ranked by relevance, not keyword match, so I find the right doc even if my wording differs.
5. **As a support engineer**, I want to record what I actually did to resolve the issue, so future engineers benefit from it (the resolution becomes new KB signal).
6. **As a support engineer**, I want the app to still work (in a degraded form) if the AI provider is down or rate-limited, so I'm not blocked from logging tickets.
7. **(Optional) As a support lead**, I want a dashboard of ticket volume by category/priority, so I can spot trends.
...
- Evaluators will run this locally, so cloud deployment is a nice-to-have, not a requirement.
...
- Similarity search is embedding-based cosine similarity, not a full production vector DB (acceptable at this scale).
- Embeddings are generated via an external free-tier API (not run locally), so — like the classification model — they are subject to rate limits and occasional cold starts; the same graceful-degradation approach applies to both.
